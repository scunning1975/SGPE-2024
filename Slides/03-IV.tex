...
\begin{frame}
\frametitle{IV in Demand Estimation}
\begin{itemize}
\item Traditional use of IV in demand estimation assumed constant treatment effects
\item Angrist, Graddy, and Imbens' late 1990s reinterpretation of the supply and demand elasticities when estimated with IV
\item Introduces the potential outcomes framework to the problem and focuses on "compliers" in the market 
\end{itemize}
\end{frame}


\begin{frame}
\frametitle{Supply Shifter as an Instrument}
\begin{itemize}
\item Paper involved a large fish market and Katy Graddy's dissertation at Princeton where she instrumented for demand using weather conditions at sea
\item Wind and storms made it harder for fishermen to catch fish, causing a leftward shift in supply, and an increase in price
\item Price was the continuous treatment variable, and weather was the instrumental variable
\item Use price theory -- what are the shifters of supply (input prices, number of sellers, alternative uses of the same inputs) and what are the shifters in demand (preferences, income, other prices, number of buyers)
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Local Average Treatment Effect (LATE)}
\begin{itemize}
\item They showed that the coefficient you estimated when instrumenting for price to get an estimate of the price elasticity of demand was a weighted average of LATEs
\item In other words -- it was the effect of weather on complier fishermen (i.e., supply)
\item Meant that the coefficient was not some single "global elasticity" but rather weighted average of individual elasticities
\item Weights were proportional to price changes caused by weather in that example
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Heterogeneity in Demand Responses}
\begin{itemize}
\item This interpretation allowed for varying price sensitivities and captured diversity in consumer behavior and seller responses to instruments
\item But again, made external validity more challenging because to maximize profits or set optimal taxes, you need not a complier related elasticity but a global elasticity of demand
\item Also the LATE is specific to an instrument -- compliers themselves are "instrument-specific" -- so these are very local elasticity concepts
\end{itemize}
\end{frame}
...
\begin{frame}{Illustrating supply and demand in equilibrium}
...
\begin{frame}{Instrumental variable shifting supply}
